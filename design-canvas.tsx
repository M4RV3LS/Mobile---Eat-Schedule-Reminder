...
const DCCtx = React.createContext<DCContextApi | null>(null);

function dcFlatten(children: React.ReactNode): React.ReactNode[] {
  const out: React.ReactNode[] = [];
  React.Children.forEach(children, (c) => {
    if (c && typeof c === 'object' && 'type' in c && c.type === React.Fragment) {
      out.push(...dcFlatten((c as React.ReactElement<{ children?: React.ReactNode }>).props.children));
    } else {
      out.push(c);
    }
  });
...
export function DesignCanvas({ children, minScale, maxScale, style }: DesignCanvasProps): React.ReactElement {
  const [state, setState] = React.useState<CanvasState>({ sections: {}, focus: null });
  ...
  React.useEffect(() => {
    ...
    fetch('./' + DC_STATE_FILE)
      ...
      .then((saved: { sections?: Record<string, SectionState> } | null) => {
  ...
  dcFlatten(children).forEach((sec) => {
    if (!sec || typeof sec !== 'object' || !('type' in sec) || sec.type !== DCSection) return;
    ...
    dcFlatten(secEl.props.children).forEach((ab) => {
      if (!ab || typeof ab !== 'object' || !('type' in ab) || ab.type !== DCArtboard) return;
      const abEl = ab as React.ReactElement<DCArtboardProps>;
      const aid = abEl.props.id ?? abEl.props.label;
      if (aid) abs.push([aid, abEl]);
    });
    ...
    sectionMeta[sid] = {
      title: persisted.title ?? secEl.props.title,
      subtitle: secEl.props.subtitle,
      slotIds: [...kept, ...srcIds.filter((k) => !kept.includes(k))],
    };
  });

  const api = React.useMemo<DCContextApi>(() => ({
    state,
    section: (id: string) => state.sections[id] || {},
    patchSection: (id: string, p: Partial<SectionState> | ((prev: SectionState) => Partial<SectionState>)) => setState((s) => ({
      ...s,
  ...
  React.useEffect(() => {
    const onKey = (e: KeyboardEvent): void => { if (e.key === 'Escape') api.setFocus(null); };
    const onPd = (e: Event): void => {
...
function DCViewport({ children, minScale = 0.1, maxScale = 8, style = {} }: DCViewportProps): React.ReactElement {
  const vpRef = React.useRef<HTMLDivElement>(null);
  const worldRef = React.useRef<HTMLDivElement>(null);
  const tf = React.useRef({ x: 0, y: 0, scale: 1 });
  const tfKey = 'dc-viewport:' + location.pathname;
  const saveT = React.useRef<ReturnType<typeof setTimeout>>();

  const lastPostedScale = React.useRef<number>();
  ...
  React.useEffect(() => {
    ...
    const onHostMsg = (e: MessageEvent): void => {
      const d = e.data;
    ...
    vp.addEventListener('gesturestart', onGestureStart, { passive: false } as EventListenerOptions);
    vp.addEventListener('gesturechange', onGestureChange, { passive: false } as EventListenerOptions);
    vp.addEventListener('gestureend', onGestureEnd, { passive: false } as EventListenerOptions);
...
export function DCSection({ id, title, subtitle, children, gap = 48 }: DCSectionProps): React.ReactElement {
  const ctx = React.useContext(DCCtx);
  const sid = id ?? title;
  const all = React.Children.toArray(dcFlatten(children));
  const artboards = all.filter((c): c is React.ReactElement<DCArtboardProps> =>
    c != null && typeof c === 'object' && 'type' in c && (c as React.ReactElement).type === DCArtboard
  );
  ...
  const byId: Record<string, React.ReactElement<DCArtboardProps>> = Object.fromEntries(
...
// Export helper
// ─────────────────────────────────────────────────────────────
async function dcExport(node: HTMLElement, w: number, h: number, name: string, kind: 'png' | 'html'): Promise<void> {
  ...
  const scrapeCss = (href: string): void => {
  ...
  const walk = (rules: CSSRuleList, base: string): void => {
  ...
  const cloneStyled = (src: Node): Node => {
    if (src.nodeType === 8 || (src.nodeType === 1 && (src as Element).tagName === 'SCRIPT')) return document.createTextNode('');
  ...
  const clone = cloneStyled(node) as Element;
  ...
  [clone, ...clone.querySelectorAll('*')].forEach((el) => {
    ...
    while ((m = re.exec(bg))) {
      ...
      jobs.push(toDataURL(url).then((d) => {
        (el as HTMLElement).style.backgroundImage = (el as HTMLElement).style.backgroundImage.split(tok).join('url("' + d + '")');
      }));
    }
  });
  ...
  const save = (blob: Blob | null, ext: string): void => {
    if (!blob) return;
...
function DCArtboardFrame({ sectionId, artboard, label, order, onRename, onReorder, onFocus, onDelete }: DCArtboardFrameProps): React.ReactElement {
  ...
  React.useEffect(() => {
    ...
    const off = (e: Event): void => { if (!menuRef.current || !menuRef.current.contains(e.target as Node)) setMenuOpen(false); };
    document.addEventListener('pointerdown', off, true);
    return () => document.removeEventListener('pointerdown', off, true);
  }, [menuOpen]);

  const doExport = (kind: 'png' | 'html'): void => {
    setMenuOpen(false);
...
function DCEditable({ value, onChange, style, tag = 'span', onClick }: DCEditableProps): React.ReactElement {
...
function DCFocusOverlay({ entry, sectionMeta, sectionOrder }: DCFocusOverlayProps): React.ReactPortal {
  const ctx = React.useContext(DCCtx)!;
  ...
  React.useEffect(() => {
    const r = (): void => setVp({ w: window.innerWidth, h: window.innerHeight });
  ...
  const Arrow = ({ dir, onClick }: { dir: 'left' | 'right'; onClick: () => void }): React.ReactElement => (
...
export function DCPostIt({ children, top, left, right, bottom, rotate = -2, width = 180 }: DCPostItProps): React.ReactElement {
  return (
    <div style={{
      position: 'absolute',
      top,
...
declare global {
  interface Window {
    DesignCanvas: typeof DesignCanvas;
    DCSection: typeof DCSection;
    DCArtboard: typeof DCArtboard;
    DCPostIt: typeof DCPostIt;
  }
}
